...
and cliente not like 'J%' and cliente not like '%JUR';";
            ...
            $querysa1 = "update cobramas_cobra.resumen set status_aarsa='PROMESA INCUMPLIDA' 
where id_cuenta not in (select c_cont from cobramas_cobra.historia where n_prom>0 
and d_prom>=curdate()) and cliente not like 'J%' and cliente not like '%JUR'
and id_cuenta in (select c_cont from cobramas_cobra.historia where n_prom>0 
and d_prom<curdate()) 
and numero_de_cuenta not in 
(select cuenta from cobramas_cobra.pagos where fecha>last_day(curdate()-interval 1 month)) 
...
require_once 'views/resumen-mobileView.php';
mysqli_close($con);
