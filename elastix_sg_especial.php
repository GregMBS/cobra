...
select nombre_deudor,numero_de_cuenta,
    ...
    if(left(tel_1_ref_1,4)='0181',right(tel_1_ref_1,8),tel_1_ref_1))))) as t1,
    saldo_total,id_cuenta,'XX' as enlace,status_de_credito,status_aarsa
 from resumen use index (queuesort)
where cliente='".$cliente."' 
and status_de_credito not like '%o'
and not exists (select * from historia where c_tele=tel_1_ref_1 and c_msge is null)
and tel_1_ref_1+0>0
UNION
select nombre_deudor,numero_de_cuenta,
    cliente,
    ...
    if(left(tel_1_ref_2,4)='0181',right(tel_1_ref_2,8),tel_1_ref_2))))) as t1,
    saldo_total,id_cuenta,'XX' as enlace,status_de_credito,status_aarsa
 from resumen use index (queuesort)
where cliente='".$cliente."' 
and status_de_credito not like '%o'
and not exists (select * from historia where c_tele=tel_1_ref_2 and c_msge is null)
and tel_1_ref_2+0>0
UNION
select nombre_deudor,numero_de_cuenta,
    cliente,
