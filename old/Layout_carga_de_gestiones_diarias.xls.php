...
$querydate="SELECT max(d_fech) from historia 
where c_cvba='Credito Si' and d_fech<curdate();";
$resultdate=mysql_query($querydate) or die(mysql_error());
while ($answerdate=mysql_fetch_row($resultdate)) {$yesterday=$answerdate[0];}
$querymain="SELECT cuenta,DATE_FORMAT(d_fech,'%d/%m/%Y'),
DATE_FORMAT(c_hrin,'%H:%i'),cuenta_concentradora_1,
...
where d_fech='".$yesterday."'
and cliente='Credito Si' and (c_cvge<>'Milt' or status_de_credito='720s') and c_cniv is null
