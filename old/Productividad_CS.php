...
sum(status_aarsa like 'ILO%'),substring_index(status_de_credito,'-',1) 
FROM resumen 
WHERE fecha_de_actualizacion>last_day(curdate() - interval 1 month)
and cliente='Credito Si'
group by cuenta_concentradora_1
;";
$resultpre=mysql_query($querypre) or die(mysql_error());
while ($answerpre=mysql_fetch_row($resultpre)) {
    $total[$i]=$answerpre[0];
    $gestores[$i]=ceil($answerpre[0]/500);
    if ($gestores[$i]>4) {$gestores[$i]=4;};
    $cilo[$i]=$answerpre[2];
    $sdc[$i]=$answerpre[3];
    if ($sdc[$i]=='360s') {$gestores[$i]=3;}
    $i++;
    }
$queryf="SELECT count(distinct id),count(distinct id*(saldo_total=0)) 
FROM resumen left JOIN folios on id=id_cuenta 
...
while ($answerf=mysql_fetch_row($resultf)) {
    $casot[$i]=$answerf[0];
    $casoc[$i]=$answerf[1];
    if ($casot[$i]<1) {$casot[$i]=0.0001;}
    if ($casoc[$i]<1) {$casoc[$i]=0;}
    $i++;
    }
$i=0;
...
<tr>
<?php for ($j=0;$j<$i;$j++) { ?>
<th colspan=3><?php echo $sdc[$j]; ?></th>
<?php } ?>
</tr>
<tr>
<?php for ($j=0;$j<$i;$j++) { ?>
