...
where week(d_fech)=week('".$fecha."') 
and year(d_fech)=year('".$fecha."') 
and c_cont>0 group by usuaria";
$result=mysql_query($query) or die(mysql_error());
while($row = mysql_fetch_row($result)) {
$GESTOR=$row[0];$CG++;
$TURNO=$row[1];
$GESTIONES=$row[2]+0;$SG=$SG+$GESTIONES;
$CONTACTOS=$row[3]+0;$SC=$SC+$CONTACTOS;
$NOCONTACTOS=$row[4]+0;$SNC=$SNC+$NOCONTACTOS;
