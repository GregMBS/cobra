...
$queryayer="SELECT max(d_fech),year(max(d_fech)),month(max(d_fech)),day(max(d_fech)) FROM historia WHERE d_fech<'2009-06-06' and c_cvge<>'Milt';";
...
$filename="Reporte_Diario_Despachos_".date('ymd',strtotime($ayer)).".xls";
...
AND (d_prom>='2006.06-06' OR (cuenta,c_cvba) IN (select cuenta,cliente from pagos))
GROUP BY c_cont";
...
AND status_de_credito not regexp 'nactivo' AND lstatus_de_credito not regexp 'iquidado' 
...
$queryh="SELECT count(1),sum(n_prom*(d_prom>='2009-06-06' OR (c_cvba,cuenta) IN (select cliente,cuenta from pagos))) FROM historia 
where 
