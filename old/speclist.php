...
historia.n_prom, producto, estado_deudor, ciudad_deudor, resumen.cliente, id_cuenta,
...
LEFT JOIN historia ON historia.c_cont=id_cuenta
LEFT JOIN (select * from historia where n_prom>0) as tmp on tmp.c_cont=id_cuenta and tmp.d_fech>historia.d_fech
WHERE ejecutivo_asignado_call_center='".$gestor."' 
AND status_aarsa='".$status."' 
AND tmp.auto is null
GROUP BY id_cuenta
...
<body>
<button onclick="window.location='statusg.php?capt=<?php echo $capt;?>'">Regressar al Status por Gestor</button><br>
