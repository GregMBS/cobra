...
join cscg using (numero_de_cuenta)
left join cyberres cr on c_cvst=cr.dictamen 
left join cyberact on accion=c_accion 
where month(d_fech)=month(curdate() - interval 1 day) and year(d_fech)=year(curdate() - interval 1 day) 
and concat(d_fech,' ',c_hrfi)<now()
