...
if ($cliente!='todos') {$clientestr=" and c_cvba='".$cliente."' ";}
    $querymain = "select Status_aarsa AS 'STATUS',ejecutivo_asignado_call_center AS 'GESTOR',
...
group by id_cuenta ORDER BY pagos.fecha
    ;";
...
<?php
        $queryc = "SELECT distinct ejecutivo_asignado_call_center FROM resumen 
        join historia on c_cont=id_cuenta
        join nombres on ejecutivo_asignado_call_center=usuaria
        where d_fech>last_day(curdate()-interval 2 month)
        and ejecutivo_asignado_call_center not regexp '-'
        order by ejecutivo_asignado_call_center
        ";
...
<select name="fecha1">
<?php
        $queryma = "SELECT distinct fecha FROM pagos 
        where fecha>last_day(curdate()-interval 6 month)
        ORDER BY fecha";
