...
if(length(c_tele)=12 and left(c_tele,1)=4,concat('0',c_tele),c_tele)))))) as tt,
    ...
    estado_deudor,ciudad_deudor
 from resumen 
join historia on c_cont=id_cuenta
left join deadlines using (c_tele)
join dictamenes on status_aarsa=dictamen
where c_carg<>'' and deadlines.c_tele is null
and cliente in ('CrediClub','Credito Si','Surtidor del Hogar')
and c_tele>999
and status_de_credito not like '%o'
and queue like 'MENS%'
having length(trim(tt)) in (8,12,13) and tt+1>1
order by tt+1
    ;";
...
<p>MENSAGES DIRECTOS Y INDIRECTOS</p>
<table>
<tr>
<?php
