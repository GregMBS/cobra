...
$querynom='select usuaria,iniciales from nombres 
where iniciales in (select c_cvge from historia where month(d_fech)=month(curdate()))
order by usuaria';
...
count(distinct c_cont),sum(c_cvst like 'PRO% DE%'),count(1)
...
<tr><td class="heavy">MIN BREAK</td>
<?php 
$sumb=0;
for ($i=1;$i<=$dhoy;$i++) {
?>
