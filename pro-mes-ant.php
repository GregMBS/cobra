...
$query1 = "select distinct historia.d_prom,historia.cuenta,historia.n_prom,historia.c_cvge,
...
left join (select c_cont,d_fech from historia where c_cvst like 'Pro% de pag%') as tmp 
on tmp.c_cont=historia.c_cont and tmp.d_fech>historia.d_fech
where (status_aarsa like 'pro% de pag%' or status_aarsa like 'Pag%') 
and cliente='Credito Si' and status_de_credito not regexp 'nactivo' 
and month(fecha_de_asignacion)=month(curdate())-1
and month(historia.d_fech)=month(curdate())-1
and historia.d_prom>=historia.d_fech
...
<html>
<head>
<title>Promesas y propuestas del mes anterior de Credito Si</title>
<style type="text/css">
