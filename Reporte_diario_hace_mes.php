...
// pagos
                    $id                  = 0;
                    $resultParcial       = mysql_query($queryparcial);
                    $numberfieldsParcial = mysql_num_fields($resultParcial);
                    ?>
                    <table>
                        <thead>
                            <tr>
                        ...
                        <tbody>
                            ...
                            $temprow[23]         = 0;
                            $temprow[24]         = 0;
                            while ($row                 = mysql_fetch_row($resultParcial)) {
                                ...
                                if ($row[0] != $temprow[0]) {
                                    ...
                                    if ($temprow[24] >= $temprow[9]) {
                                        ...
                                        $leftover    = max($temprow[10] - $temprow[24],
                                            0);
                                        $temprow[22] = $temprow[9] - $temprow[24]
                                            - $leftover;
                                        $temprow[23] = $leftover;
                                    }
                                } else {
...
// vencidos
                    ...
                    $resultVencidos       = mysql_query($queryvencido);
                    $numberfieldsVencidos = mysql_num_fields($resultVencidos);
                    echo "<table>";
                    echo "<tr>";
                    for ($i = 0; $i < $numberfieldsVencidos; $i++) {
                        $var = mysql_field_name($resultVencidos, $i);
                        echo "<th>".$var."</th>";
                    }
                    ?>
                    <th>monto1</th>
                ...
                <?php
                while ($row = mysql_fetch_row($resultVencidos)) {
                    if ($row[0] <> $id) {
                        $id = $row[0];
...
values ('".$row[15]."','".$row[2]."','".$row[3].
        ...
        <div id='vigentes'>
            <?php
// vigentes
            $resultVigentes       = mysql_query($queryvigente);
            $numberfieldsVigentes = mysql_num_fields($resultVigentes);
            echo "<table>";
            echo "<tr>";
            for ($i = 0; $i < $numberfieldsVigentes; $i++) {
                $var = mysql_field_name($resultVigentes, $i);
                echo "<th>".$var."</th>";
            }
            ?>
            <th>pmt1</th>
...
from gmbtemp group by cliente;";
            $resultSumCliente       = mysql_query($querySumCliente);
...
from gmbtemp group by gestor;";
            $resultSumGestor       = mysql_query($querySumGestor);
...
from gmbtemp group by cliente,substring_index(sdc,'-',1);";
            $resultSumStatus       = mysql_query($querySumStatus);
...
from gmbtemp group by cliente,substring_index(sdc,'-',1),producto,subproducto;";
            $resultSumProducto       = mysql_query($querySumProducto);
