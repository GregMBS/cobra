            ...
            <div style='float:left;width:40%'><label>
                    <select name='queue'>
                        ...
                        foreach ($queues as $queue) {
                            ?>
                            <option value='<?php
                            echo $queue->cliente . ',' . $queue->sdc . ',' . $queue->getStatusAarsa();
                            ?>' <?php
                            echo $queue->getBloqueado();
                            ?>><?php
                                echo $queue->cliente . '-' . $queue->sdc . '-' . $queue->getStatusAarsa();
                                ?>
                            </option>
    ...
    foreach ($resultList as $rowList) {
        ...
        <div style='clear:both;border:1pt black solid'>
            <form method='get' action='/queues.php' name='<?php echo $rowList->INICIALES; ?>'>
                <div style='float:left;width:25%'>
                    <label><input name='gestor' type='text' readonly='readonly'
                                  value='<?php echo $rowList->INICIALES; ?>'></label>
                </div>
