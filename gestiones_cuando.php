...
<html>
<head>
<title>Reporte semanal de Gestion</title>
...
count(1) as 'Gestiones Totales',sum((c_carg<>'')*(c_carg<>'otro')) as 'Contactos',
count(1)-sum((c_carg<>'')*(c_carg<>'otro')) as 'No Contactos',
sum(c_carg<>'')/count(1) as '% de Penetracion',
...
$PCP=number_format($row[5]*100,0);$SPCP=number_format($SC/($SG+0.001)*100,0);
$PTP=$row[6];$SPTP=$SPTP+$PTP;
$PCPTP=number_format($row[7]*100,0);$SPCPTP=number_format($SPTP/($SC+0.001)*100,0);
?>
<tr>
...
$CONTACTOS=$row[3];$SC=$SC+$CONTACTOS;
$NOCONTACTOS=$row[4];$SNC=$SNC+$NOCONTACTOS;
$PCP=number_format($row[5]*100,0);
$PTP=$row[6];$SPTP=$SPTP+$PTP;
$PCPTP=number_format($row[7]*100,0);
?>
<tr>
<td><?php echo $GESTOR;?></td>
<td><?php echo $TURNO;?></td>
...
<?php } 
$SPCP=number_format($SC/($SG+0.001)*100,0);
$SPCPTP=number_format($SPTP/($SC+0.001)*100,0);
?>
<tr>
<th><?php echo $CG;?> GESTORES</th>
<th><?php echo $CT;?> TURNOS</th>
