...
<?php
                        $queryres = "show columns from sdhextras";
...
and field not regexp 'cuenta';";
            ...
            {
                $fieldlist = $fieldlist . $sep . 'sdhextras.' . $answerfcont[0] . '=inftemp.' . $answerfcont[0];
                $sep = ',';
            }
            $ncr='';
            $ncr1='';
            $ncr2='';
            $queryioff="ALTER TABLE sdhextras DISABLE KEYS";
            $queryion="ALTER TABLE sdhextras ENABLE KEYS";
            //mysql_query($queryioff) or die(mysql_error());
            $queryupd2 = "UPDATE inftemp, sdhextras 
	        SET " . $fieldlist . " 
            where inftemp.cuenta=sdhextras.cuenta;";
            ...
            $queryins = "insert into sdhextras (" . $fieldlist . ") select " . $fieldlist . " from inftemp 
            where cuenta+0>0 and not exists (
            select * from sdhextras 
            where inftemp.cuenta=sdhextras.cuenta);";
