    ...
    if ($answercheck[0] != 1) {
        ...
        <html>
            ...
            <body>
                ...
                if (!empty($post['go'])) {
                    ...
                    if ($post['go'] == 'clientepick') {
                        ...
                        <form action="carga2.php" method="post" name="assoc">
                            <?php
                            $cliente = mysql_real_escape_string($post['cliente']);
                            ...
                            if (isset($post['fecha_de_actualizacion'])) {
                                $fecha_de_actualizacion = mysql_real_escape_string($post['fecha_de_actualizacion']);
                            }
                            $filename = mysql_real_escape_string($post['filename']);
                ...
                if ($post['go'] == 'asociar') {
                    $maxc                   = mysql_real_escape_string($post['maxc']);
                    $cliente                = mysql_real_escape_string($post['cliente']);
                    $fecha_de_actualizacion = mysql_real_escape_string($post['fecha_de_actualizacion']);
                    $filename               = mysql_real_escape_string($post['filename']);
...
//                                           $position[$k] = $k;
                        ...
                        for ($f = 0; $f < $maxc; $f++) {
                            $pos = mysql_real_escape_string($post['pos'.$f]);
...
//            die($querystart.$queryend);
                    ...
                    $filename2  = mysql_real_escape_string($post['filename']);
                    $n          = 0;
