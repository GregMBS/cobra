...
where d_fech>last_day(curdate()-interval 2 month)
and d_fech<=last_day(curdate()-interval 1 month)) as tmp";
...
<html>
<head>
<title>Visitas del Mes Anterior</title>
...
<body>
<h2>VISITAS DEL MES ANTERIOR</h2>
<table summary="LpH">
<?php 
for ($i=1;$i<=$dhoy;$i++) {
	$tsumt[$i]=0;
...
and c_cniv is not null and year(D_FECH)=year(curdate()-interval 2 month) 
...
from vasign 
where gestor='$c_visit' 
and fechain>last_day(curdate()-interval 2 month)
and fechain<last_day(curdate()-interval 1 month)+interval 1 day
and day(fechain)=".$i;
