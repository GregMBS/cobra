...
left join folios on c_cont=id and not exists (select folio from folios f2 where folios.id=f2.id and folios.folio<f2.folio)
where n_prom>0 and d_fech>last_day(curdate()-interval 2 month) 
and d_fech<=last_day(curdate()-interval 1 month) and d_prom<curdate()
and not exists 
(select auto from historia h2 
where h1.c_cont=h2.c_cont and h1.auto<h2.auto and h2.n_prom>0) 
and not exists 
(select auto from pagos 
where h1.c_cont=id_cuenta and fecha>last_day(curdate()-interval 2 month)  and fecha<last_day(curdate()-interval 1 month)) 
;";
...
and d_fech<=last_day(curdate()-interval 1 month) and d_fech<=fecha
and status_aarsa<>'PAGO TOTAL' and confirmado=0 and fecha<last_day(curdate()-interval 1 month)
and not exists 
(select auto from historia h2 
...
from historia h1,pagos,resumen
where c_cont=pagos.id_cuenta and c_cont=resumen.id_cuenta
and n_prom>0 and d_fech>last_day(curdate()-interval 2 month) and d_fech<=last_day(curdate()-interval 1 month) and d_fech<=fecha
and status_aarsa<>'PAGO TOTAL' and confirmado=0 and fecha<last_day(curdate()-interval 1 month)
and not exists 
(select auto from historia h2 
where h1.c_cont=h2.c_cont and h1.auto<h2.auto and h2.n_prom>0  and h2.d_fech<=fecha) 
...
from historia h1
where n_prom>0 and d_fech>last_day(curdate()-interval 2 month) and d_fech<=last_day(curdate()-interval 1 month) and d_prom1<curdate()
and not exists 
(select auto from historia h2 
where h1.c_cont=h2.c_cont and h1.auto<h2.auto and h2.n_prom>0) 
and not exists 
...
from historia h1
where n_prom>0 and d_fech>last_day(curdate()-interval 2 month) and d_fech<=last_day(curdate()-interval 1 month) and d_prom2<curdate()
and not exists 
(select auto from historia h2 
where h1.c_cont=h2.c_cont and h1.auto<h2.auto and h2.n_prom>0) 
and not exists 
...
from historia h1
where n_prom>0 and d_fech>last_day(curdate()-interval 2 month) and d_fech<=last_day(curdate()-interval 1 month) 
and not exists 
(select auto from historia h2 
where h1.c_cont=h2.c_cont and h1.auto<h2.auto and h2.n_prom>0) 
and not exists 
