...
function validate_form(tf, evt, minprom, authorized) {
    ...
    var n1 = 0;
    var n2 = 0;
    var np = parseFloat(tf.N_PROM1.value) + parseFloat(tf.N_PROM2.value);
    var cvst = "";
    var ccn = " ";
    var cnt = "";
    var co2 = "";
    ...
    if (typeof (tf.C_CVGE.value) !== "undefined") {
        gestor = tf.C_CVGE.value;
    }
    ...
    if (gestor == '') {
        ...
        flag = 1;
        alerttxt = alerttxt + 'No hay gestor.';
    }
    ...
    // Must have accion
    if (validate_required(tf.C_ACCION) === false) {
        alerttxt = alerttxt + 'ACCION es necesario\n';
        tf.C_ACCION.style.backgroundColor = "yellow";
        flag = 1;
    }
    ...
    if (cvst) {
        ...
        // promesa
        // less than minprom
        if (cvst == "PROMESA DE PAGO PARCIAL") {
            if (validate_required(tf.C_CARG) === false) {
                alerttxt = alerttxt + "Carga/Parentesco es necesario" + '\n';
                tf.C_CNP.style.backgroundColor = "yellow";
                flag = 1;
        ...
        if (cvst.substr(0, 12) === "PROPUESTA DE") {
            if (n1 < 10) {
                alerttxt = alerttxt + "MONTO DE PROMESA es necesario" + '\n';
                tf.N_PROM1.style.backgroundColor = "yellow";
                tf.N_PROM2.style.backgroundColor = "yellow";
    ...
    if (cvst) {
        ...
        if (cvst.substr(0, 11) === "MENSAJE EN ") {
            if (tf.C_CARG.value.length !== 0) {
                alerttxt = alerttxt
                    + 'Cargo del contacto no es necesario cuando STATUS es '
                    + cvst;
    ...
    if (n1 > 0) {
        ...
        if (authorized < 1) {
            ...
            if (ccn == "PROMESA DE PAGO TOTAL") {
                alerttxt = alerttxt
                    + "Solamente un supervisor puede sobrescribir una promesa activa.";
                flag = 1;
            }
    ...
    var alertstr = 'Gestion de cuenta ' + tf.CUENTA.value + ' guardado con status '
        + cvst + ".\n";
    if (n1 > 0) {
    ...
    if (n2 > 0) {
        alertstr = alertstr + " Fecha de promesa 2: " + dp2 + " ";
    }
    if (n2 > 0) {
        alertstr = alertstr + " Monto de promesa 2: $" + n2 + "\n";
    }
    if (npa > 0) {
