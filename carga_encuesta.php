...
<?php
    ...
    if (!empty($_POST['go'])) 
    {
        ...
        if ($_POST['go'] == 'cargar') 
        {
            ...
            $querytemp1 = 'truncate encuesta';
mysql_query($querytemp1) or die(mysql_error());
$data = preg_split("/[\s,]+/", $_POST['data'],0,PREG_SPLIT_NO_EMPTY);
$max=count($data);
$queryload='';
for ($i=0;$i<$max;$i++) {
$queryload = "INSERT INTO encuesta (id) VALUES ('".$data[$i]."');";
mysql_query($queryload) or die(mysql_error());
}
?>
<p>Cuentas estan guardado</p>
<?php } } ?>
<button onclick="window.location='encuesta_pdf.php?capt=<?php echo $capt;?>'">Imprimir encuesta
</button><br>
