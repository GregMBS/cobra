...
$field='id_cuenta';
$find=$C_CONT;
$TAG0=mysql_real_escape_string ($_GET['SDC']);
$TAGA=explode('-',$TAG0);
$TAG=trim($TAGA[0]);
$TAGS=$TAG;
$INACTIVO=!empty($_GET['inactivo']);
$LIQUIDADO=!empty($_GET['liquidado']);
if ($TAG!='') {
if ($INACTIVO) {$TAGS=$TAG.'-inactivo';}
if ($LIQUIDADO) {$TAGS=$TAG.'-liquidado';}
}
else {
if ($INACTIVO) {$TAGS=$TAG.'Inactivo';}
if ($LIQUIDADO) {$TAGS=$TAG.'Liquidado';}
...
$STATUS=$row[4];
$STATUSC=$row[5];
if (preg_match('/activo$/',$STATUS)) {$INACTIVO=1;} else {$INACTIVO=0;}
if (preg_match('/quidado$/',$STATUS)) {$LIQUIDADO=1;} else {$LIQUIDADO=0;}
?>
<tr>
...
INACTIVO<input type="checkbox" name="inactivo" value="inactivo"<?php if ($INACTIVO==1) {?> checked=checked<?php } ?>><br>
LIQUIDADO<input type="checkbox" name="liquidado" value="liquidado"<?php if ($LIQUIDADO==1) {?> checked=checked<?php } ?>><br>
